...
config.delQualityUser = function () {
    localStorage.removeItem("WeldStaffUser");
}
//用户有效性检查
config.userValid = function (name, password, cb) {
    var user = {'name': name, 'pass': password};
    config.ajaxGet("/weldstaffuser/find", {data: JSON.stringify(user)}, function (res) {
...
config.userCheck = function (cb) {
    var userObj = config.localParamGet("WeldStaffUser");
    if (userObj == null) {
        //无登陆信息
...
config.loadConfigParam = function(cb) {
        if (config.ConfigParam) {
            ...
            //加载配置
            $.getJSON("../diction/get", function (data) {
