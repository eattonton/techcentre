(function($) {
  ...
  $.extend($.fn.datagrid.methods, {
    autoSizeColumn: function(jq, field) {
      return jq.each(function() {
        var fc = $(this)
          .datagrid("getPanel")
          .find(".datagrid-header .datagrid-filter-c");
        // fc.hide();
        fc.css({
          width: "1px",
          height: 0
    ...
    insert: function(jq, param) {
      return jq.each(function() {
        ...
        if (opts.oldLoadFilter) {
          ...
          var pid =
            index >= 0 ? state.filterSource.rows[index]._parentId : null;
          var rows = translateTreeData(this, [param.data], pid);
          var newRows = state.filterSource.rows.splice(
            0,
            index >= 0
              ? param.before
                ? index
                : index + 1
              : state.filterSource.rows.length
          );
    ...
    filterMatcher: function(data) {
      ...
      function isMatch(row, index) {
        ...
        for (var i = 0; i < rules.length; i++) {
          ...
          var formattedValue =
            col && col.formatter
              ? col.formatter(row[rule.field], row, index)
              : undefined;
          ...
          if (value != "") {
            if ((rule && rule.value != value) || !rule) {
              var op = rule
                ? rule.op
                : filterOpts
                ? filterOpts.defaultFilterOperator || opts.defaultFilterOperator
                : opts.defaultFilterOperator;
    ...
    endwith: {
      ...
      isMatch: function(source, value) {
        ...
        return (
          source
            .toLowerCase()
            .indexOf(value.toLowerCase(), source.length - value.length) !== -1
        );
      }
    },
    less: {
  ...
  function resizeFilter(target, field) {
    ...
    var ff = field
      ? header.find('.datagrid-filter[name="' + field + '"]')
      : header.find(".datagrid-filter");
    ff.each(function() {
  ...
  function getFilterComponent(target, field) {
    ...
    return header.find(
      'tr.datagrid-filter-row td[field="' + field + '"] .datagrid-filter'
    );
  }
  ...
  function doFilter(target) {
    ...
    if (opts.remoteFilter) {
      ...
      if (
        opts.view.type == "scrollview" &&
        state.data.firstRows &&
        state.data.firstRows.length
      ) {
        state.data.rows = state.data.firstRows;
      }
      $(target)
        [name]("getPager")
        .pagination("refresh", { pageNumber: 1 });
  ...
  function translateTreeData(target, children, pid) {
    ...
    $.map(children, function(item) {
      ...
      rows.push(item);
      rows = rows.concat(
        translateTreeData(target, item.children, item[opts.idField])
      );
    });
  ...
  function init(target, filters) {
    ...
    state.dc.view2
      ...
      .unbind(".filter")
      .bind("focusin.filter", function(e) {
        var header = $(this);
        setTimeout(function() {
    ...
    function createFilter(frozen) {
      ...
      var table = (frozen ? dc.header1 : dc.header2).find(
      ...
      for (var i = 0; i < fields.length; i++) {
        ...
        if (!div) {
          ...
          var input = filter.init(
            div,
            $.extend({ height: opts.editorHeight }, fopts.options || {})
          );
  ...
  $.extend($.fn.datagrid.methods, {
    getFilterValues: function(jq) {
      var values = {};
      jq.each(function() {
        var name = getPluginName(this);
        var opts = $.data(this, name).options;
        if (opts.filterCache) {
          for(var key in opts.filterCache){
            var value = opts.filterCache[key];
            var _val = value ? value.find("input").eq(0).val():"";
            _val &&  (values[key] = _val);
          }
        }
      });
      return values;
    ...
    disableFilter: function(jq) {
      return jq.each(function() {
        ...
        if (!div.length) {
          div = $('<div class="datagrid-filter-cache"></div>').appendTo(
            dc.view
          );
        }
    ...
    destroyFilter: function(jq, field) {
      return jq.each(function() {
        ...
        if (field) {
          ...
          $(this)
            .datagrid("getPanel")
            .find(".datagrid-header .datagrid-filter-row")
            .remove();
          $(this)
            .data("datagrid")
            .dc.view.children(".datagrid-filter-cache")
            .remove();
          opts.filterCache = {};
          $(this)[name]("resize");
          $(this)[name]("disableFilter");
